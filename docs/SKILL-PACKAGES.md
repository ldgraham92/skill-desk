...
Suggestions come only from the bundled AI Hero and PStack collections and exclude skills already available to the destination agent. A skill installed only for Claude can still be recommended for Codex, and vice versa. For a project, existing skills include its selected agent’s project skills and personal skills; unrelated projects are excluded. Each suggestion includes its reason and the reviewed prompts that support it. Choose **Review & install** to open its instructions, check the destination, then confirm with **Install for Codex** or **Install for Claude Code**. The suggestion is marked **Installed** afterward. **More options → Save for later** keeps a suggestion without installing it. Recommendations never install skills automatically.
...
Project selection changes the destination, skills compared for overlap, and default history scope. **This project only** includes prompts with matching absolute repository metadata, including subdirectories. Sibling repositories and separately registered worktrees remain separate. Prompts with missing metadata are excluded; the preview explains incomplete or empty samples. Nested repositories are excluded when their Git marker is present locally. **All projects** is an explicit alternative and the default for personal libraries. Changing scope clears the displayed preview. Filtering uses local session metadata, never a guess from prompt text.

Codex samples recognize both CLI user-message events and desktop response-item user messages. Subagent sessions and injected context such as plugin lists and AGENTS.md instructions are excluded. Large sessions contribute bounded opening and recent sections, so long stretches of tool output do not hide every user prompt. Codex samples use session and turn working directories. Claude samples use history project paths and per-message working directories. Truncated Codex sessions require fresh directory context after the skipped section. Global Codex history without directory metadata does not enter a project-only sample. These checks can omit otherwise relevant prompts; they do not silently widen the scope.
...
## Compare and replace one skill

A single-skill preview with a destination conflict offers **Compare with destination copy**. The comparison lists added, removed, and changed files and shows the `SKILL.md` diff. Supporting files appear in the file list; their contents are not executed. Explicitly confirm the comparison to replace the copy. Skill-Desk checks the installed tree again and refuses replacement if it changed after review.

The previous folder is archived before installation. A failed replacement restores it. The archive remains listed under Removed skills. Restore is blocked while another skill occupies the destination; remove or undo the replacement before restoring the previous copy. Linked library locations and linked skill trees cannot be replaced through this flow. Package imports continue to skip conflicts rather than replacing them in bulk.

Repeated installation requests for the same candidate return the completed result. A partially completed package can be retried without creating duplicate installation records. Nearby transfers can be retried after a failed checksum or interrupted connection; successful reception still requires a separate installation preview.

## Draft editing and copies

Single-skill previews offer **Edit instructions**, **Revise with agent**, **Compare revision**, and **Save draft locally**. Validation checks metadata and entrypoint references before edits apply. Supporting Markdown may refer to another skill; missing/external links there appear as review warnings. Text files can be previewed up to 200 KB. Binary files are preserved but not rendered.

A successful agent revision creates a separate preview and retains the prior preview. **Add skills → Saved drafts** lists both current-session previews and explicitly saved drafts. Saved drafts survive app restart; unsaved previews and edit history do not. Reopening checks that the destination remains registered. Deleting a saved draft does not remove installed skills.

Installed skill details include **Duplicate as new skill**, **Compare copies**, and **Copy to agents**. Duplication preserves supporting files and asks for a new name. Comparisons identify equal file contents or show an instructions diff and changed file lists. Copying to multiple agents requires reviewing each destination; existing copies are skipped and failed copies can be retried. The copies remain independent.
