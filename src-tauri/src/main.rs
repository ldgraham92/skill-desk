    ...
    let app = tauri::Builder::default()
        ...
        .setup(|app| {
            let window = WebviewWindowBuilder::new(app, "main", WebviewUrl::App("index.html".into()))
                ...
                .icon(app.default_window_icon().unwrap().clone())?.build()?;
            ...
            TrayIconBuilder::new().icon(app.default_window_icon().unwrap().clone())
                ...
                .on_menu_event(|app, event| match event.id.as_ref() { "open" => show(app), "hide" => { if let Some(w) = app.get_webview_window("main") { let _ = w.hide(); } }, "quit" => { stop(app); app.exit(0); }, _ => {} })
                .build(app)?;
        ...
        .on_window_event(|window, event| {
            if let tauri::WindowEvent::CloseRequested { .. } = event { stop(window.app_handle()); window.app_handle().exit(0); }
        })
